...
class Query(graphene.ObjectType):
    ...
    user_scheduled_stories = graphene.List(StoryType)
    ...
    def resolve_user_scheduled_moments(self, info, **kwargs):
        user = info.context.user
        return Moment.objects.all().filter(
            user=user, is_published=False
        ).order_by(F('publish_at').desc(nulls_last=True), '-created_date')
    ...
    def resolve_user_scheduled_stories(self, info, **kwargs):
        user = info.context.user
        user_stories = Story.objects.all().filter(user=user, is_published=False).order_by(F('publish_at').desc(nulls_last=True), '-created_date')
    ...
    # def resolve_user_scheduled_stories(self, info):
    #     return Story.objects.filter(is_published=True).all()
