...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_owned_by(self, obj):
        ...
        return UserOwnerSerializer(data, many=True).data if data else []
...
class OwnedBySerializer(serializers.ModelSerializer):
    ...
    def get_id(self, instance):
        return instance.owned_by.first().id

    def get_username(self, instance):
        return instance.owned_by.first().username

    def get_first_name(self, instance):
        return instance.owned_by.first().first_name

    def get_last_name(self, instance):
        return instance.owned_by.first().last_name

    def get_fullName(self, instance):
        return instance.owned_by.first().fullName

class AllModeratorSerializer(serializers.ModelSerializer):
    avatar_photos = UserPhotoSerializer(many=True, read_only=True)
    roles = serializers.SlugRelatedField(slug_field="role", many=True, read_only=True)
    tags = serializers.PrimaryKeyRelatedField(
        many=True, queryset=pickerModels.tags.objects.all()
    ...
    class Meta:
        model = User
        exclude = ('owned_by','password','user_permissions', 'username',)
