...
class ModerateListView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = serializers.OwnedBySerializer

    def get_queryset(self):
        user_id = self.kwargs.get('user_id')

        role_users = User.objects.filter(id=user_id)
        return role_users


class AllModeratorByOwnerListView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = serializers.AllModeratorSerializer

    def get_queryset(self):
        owner_id = self.kwargs.get('owner_id')
        users = User.objects.filter(owned_by__id=owner_id)
        return users