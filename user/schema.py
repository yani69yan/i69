...
from chat.serializer import ChatUserType
...
class Query(graphene.ObjectType):
    ...
    usersAdmins = graphene.List(ChatUserType)
    usersCreatedByAdmin = graphene.List(ChatUserType, id=graphene.String(required=True))
    ...
    def resolve_usersAdmins(self, info):
        user=info.context.user
        try:
            return User.objects.filter(Q(roles__role__in=["ADMIN"]) | Q(roles__role__in=["SUPER_ADMIN"])).all()
        except:
            raise Exception(translate_error_message(
                user,"try again"))

    def resolve_usersCreatedByAdmin(self, info, id=None):
        user=info.context.user
        try:
            if not id:
                raise Exception(translate_error_message(
                user, "Admin id not provided"))
            return User.objects.filter(Q(owned_by__id__in=[id])).all()
        except:
            raise Exception(translate_error_message(
                user, "try again"))
